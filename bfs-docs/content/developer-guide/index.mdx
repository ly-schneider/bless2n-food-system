...
- `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` setzen; `BACKEND_INTERNAL_URL` verweist lokal auf `http://localhost:8080`.
- `pnpm install && pnpm dev` in `bfs-web-app`.
- Alle API‑Calls gehen über gleich‑origin `/api`; Server‑Routen sprechen per `BACKEND_INTERNAL_URL` direkt mit dem Backend.
...
- SMTP Konfiguration mit entwicklungsfreundlichen TLS Policies; Dev‑Preview‑Endpoints für Staging‑Builds.
...
- API Proxy: `/lib/api.ts` nutzt gleich‑origin `/api` und leitet mithilfe von `BACKEND_INTERNAL_URL` zum Backend.
