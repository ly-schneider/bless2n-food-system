...
func (h *AdminUserHandler) Promote(w http.ResponseWriter, r *http.Request) {
	...
	if u.Role != domain.UserRoleCustomer {
		response.WriteError(w, http.StatusBadRequest, "unsupported role change")
		return
	}
	if err := h.users.UpdateRole(r.Context(), oid, domain.UserRoleAdmin); err != nil {
...
func (h *AdminUserHandler) PatchRole(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.users.UpdateRole(r.Context(), oid, role); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "update failed")
		return
	}
	response.WriteJSON(w, http.StatusOK, map[string]any{"ok": true})
...
func (h *AdminUserHandler) PatchProfile(w http.ResponseWriter, r *http.Request) {
	...
	if body.FirstName != nil || body.LastName != nil {
		if err := h.users.UpdateNames(r.Context(), oid, body.FirstName, body.LastName); err != nil {
			response.WriteError(w, http.StatusInternalServerError, "update names failed")
			return
		}
		// refresh
		if nu, err := h.users.FindByID(r.Context(), oid); err == nil && nu != nil {
			u = nu
		}
	}
	// Email / verification
	if body.Email != nil || body.IsVerified != nil {
		...
		if body.IsVerified != nil {
			newVerified = *body.IsVerified
		}
		if err := h.users.UpdateEmail(r.Context(), oid, newEmail, newVerified); err != nil {
			response.WriteError(w, http.StatusInternalServerError, "update email failed")
			return
		}
		if nu, err := h.users.FindByID(r.Context(), oid); err == nil && nu != nil {
			u = nu
	...
	if body.Role != nil {
		...
		if err := h.users.UpdateRole(r.Context(), oid, domain.UserRole(*body.Role)); err != nil {
			response.WriteError(w, http.StatusInternalServerError, "update role failed")
			return
		}
		if nu, err := h.users.FindByID(r.Context(), oid); err == nil && nu != nil {
			u = nu
