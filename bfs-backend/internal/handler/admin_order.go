...
func (h *AdminOrderHandler) ExportCSV(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "failed to export")
		return
	}
	...
	for _, o := range items {
		_ = cw.Write([]string{o.ID.Hex(), string(o.Status), strconv.FormatInt(int64(o.TotalCents), 10), o.CreatedAt.Format(time.RFC3339)})
	}
	cw.Flush()
}

type patchOrderStatus struct {
	Status domain.OrderStatus `json:"status"`
}
...
func (h *AdminOrderHandler) PatchStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.orders.UpdateStatusAndContact(r.Context(), oid, body.Status, nil); err != nil {
...
func (h *AdminOrderHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	var custID *string
	if ord.CustomerID != nil {
		s := ord.CustomerID.Hex()
		custID = &s
	}
