...
func (h *AdminStationHandler) ListStationProducts(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "failed to list assignments")
		return
	}
	...
	for _, p := range prods {
		out = append(out, row{ProductID: p.ID.Hex(), Name: p.Name})
	}
	response.WriteJSON(w, http.StatusOK, map[string]any{"items": out})
}
...
func (h *AdminStationHandler) AssignProducts(w http.ResponseWriter, r *http.Request) {
	...
	if len(ids) == 0 {
		response.WriteError(w, http.StatusBadRequest, "no valid product ids")
		return
	}
	added, err := h.stationProds.AddProducts(r.Context(), oid, ids)
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "assign failed")
		return
	}
...
func (h *AdminStationHandler) RemoveProduct(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "remove failed")
		return
	}
