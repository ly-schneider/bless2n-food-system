...
func (h *UserHandler) RequestEmailChange(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { response.WriteProblem(w, response.NewProblem(http.StatusBadRequest, http.StatusText(http.StatusBadRequest), "Invalid user id")); return }
...
func (h *UserHandler) ConfirmEmailChange(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&body); err != nil { response.WriteProblem(w, response.NewProblem(http.StatusBadRequest, http.StatusText(http.StatusBadRequest), "Invalid JSON payload")); return }
    if err := h.validator.Struct(body); err != nil {
        if verrs, ok := err.(validator.ValidationErrors); ok { response.WriteProblem(w, response.NewValidationProblem(response.ConvertValidationErrors(verrs), r.URL.Path)); return }
        response.WriteProblem(w, response.NewProblem(http.StatusBadRequest, http.StatusText(http.StatusBadRequest), "Validation failed")); return
    }
    oid, err := primitive.ObjectIDFromHex(claims.Subject)
    if err != nil { response.WriteProblem(w, response.NewProblem(http.StatusBadRequest, http.StatusText(http.StatusBadRequest), "Invalid user id")); return }
