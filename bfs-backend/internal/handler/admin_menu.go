...
func (h *AdminMenuHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "failed to list menus")
		return
	}
	type MenuDTO struct {
		ID         string  `json:"id"`
	...
	for _, m := range menus {
		out = append(out, MenuDTO{ID: m.ID.Hex(), Name: m.Name, PriceCents: int64(m.PriceCents), IsActive: m.IsActive, Image: m.Image})
	}
...
func (h *AdminMenuHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || m == nil || m.Type != domain.ProductTypeMenu {
	...
	type SlotDTO struct {
		...
		MenuSlotItems []SlotItemDTO `json:"menuSlotItems"`
	}
...
func (h *AdminMenuHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.Name == "" || body.PriceCents < 0 || body.CategoryID == "" {
	...
	cat, err := h.categories.GetByID(r.Context(), catID)
	if err != nil || cat == nil {
...
func (h *AdminMenuHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if body.PriceCents != nil && *body.PriceCents >= 0 {
		set["price_cents"] = *body.PriceCents
	}
	if body.Image != nil {
		if *body.Image == "" {
			set["image"] = nil
	...
	if err := h.products.UpdateFields(r.Context(), oid, set); err != nil {
...
type patchMenuActiveBody struct {
	IsActive bool `json:"isActive"`
}
...
func (h *AdminMenuHandler) DeleteHard(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.products.DeleteByID(r.Context(), oid); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "delete failed")
		return
	}
...
func (h *AdminMenuHandler) CreateSlot(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "create slot failed")
		return
	}
...
func (h *AdminMenuHandler) RenameSlot(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.slots.UpdateName(r.Context(), soid, body.Name); err != nil {
...
func (h *AdminMenuHandler) ReorderSlots(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.slots.UpdateSequences(r.Context(), seqs); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "reorder failed")
		return
	}
...
func (h *AdminMenuHandler) DeleteSlot(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.items.DeleteByMenuSlotID(r.Context(), soid); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "delete items failed")
		return
	}
	if err := h.slots.DeleteByID(r.Context(), soid); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "delete slot failed")
		return
	}
...
func (h *AdminMenuHandler) AttachItem(w http.ResponseWriter, r *http.Request) {
	...
	if p.Type != domain.ProductTypeSimple {
		response.WriteError(w, http.StatusBadRequest, "only simple products can be attached")
		return
	}
	exists, err := h.items.ExistsBySlotAndProduct(r.Context(), soid, pid)
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "check failed")
		return
	}
	if exists {
		response.WriteJSON(w, http.StatusOK, map[string]any{"ok": true})
	...
	if err := h.items.Insert(r.Context(), &domain.MenuSlotItem{MenuSlotID: soid, ProductID: pid}); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "attach failed")
		return
	}
...
func (h *AdminMenuHandler) DetachItem(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.items.DeleteBySlotAndProduct(r.Context(), soid, pid)
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "detach failed")
		return
	}
