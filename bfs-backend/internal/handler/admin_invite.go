...
func (h *AdminInviteHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "failed to list invites")
		return
	}
...
func (h *AdminInviteHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	token, err := utils.GenerateRandomURLSafe(32)
	if err != nil {
...
func (h *AdminInviteHandler) Revoke(w http.ResponseWriter, r *http.Request) {
	...
	ok, err := h.invites.Revoke(r.Context(), oid)
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "revoke failed")
		return
	}
	if !ok {
		response.WriteError(w, http.StatusBadRequest, "not pending")
		return
	}
...
func (h *AdminInviteHandler) Accept(w http.ResponseWriter, r *http.Request) {
	...
	if inv.Status != "pending" || time.Now().UTC().After(inv.ExpiresAt) {
		response.WriteError(w, http.StatusUnauthorized, "expired or used")
		return
	}
	// Require name to create admin user
	if body.FirstName == nil || *body.FirstName == "" {
		response.WriteError(w, http.StatusBadRequest, "firstName required")
		return
	}
	...
	if err != nil {
		response.WriteError(w, http.StatusInternalServerError, "user create failed")
		return
	}
	if err := h.invites.MarkAccepted(r.Context(), inv.ID); err != nil {
		response.WriteError(w, http.StatusInternalServerError, "mark accepted failed")
		return
	}
	...
	if h.jwt != nil && h.refresh != nil {
		...
		if clientID == "" {
			clientID = r.Header.Get("X-Original-User-Agent")
		}
		if clientID == "" {
			clientID = r.Header.Get("X-Client-UA")
		}
		if clientID == "" {
		...
		if _, err := h.refresh.Create(r.Context(), &domain.RefreshToken{
			...
			response.WriteError(w, http.StatusInternalServerError, "session error")
			return
		}
...
func (h *AdminInviteHandler) Resend(w http.ResponseWriter, r *http.Request) {
	...
	if inv.Status != "pending" {
		response.WriteError(w, http.StatusBadRequest, "invite not pending")
		return
	}
	token, err := utils.GenerateRandomURLSafe(32)
	if err != nil {
	...
	if err := h.invites.UpdateToken(r.Context(), inv.ID, hash, exp); err != nil {
...
func (h *AdminInviteHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.invites.Delete(r.Context(), oid); err != nil {
