...
// PreviewLoginEmail renders the login email (HTML by default, or text when format=text) for preview in non-production.
func (h *DevHandler) PreviewLoginEmail(w http.ResponseWriter, r *http.Request) {
	// Only allow when not production
	if strings.EqualFold(h.cfg.App.AppEnv, "production") {
		http.NotFound(w, r)
		return
	}
	q := r.URL.Query()
	...
	code := q.Get("code")
	if code == "" {
		if c, err := utils.GenerateOTP(); err == nil {
			code = c
		} else {
			code = "123456"
		}
...
// PreviewEmailChangeEmail renders the email-change verification email for preview in non-production.
...
// PreviewAdminInviteEmail renders the admin invite email for preview in non-production.
