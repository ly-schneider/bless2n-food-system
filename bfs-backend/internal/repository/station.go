...
func (r *stationRepository) FindByDeviceKey(ctx context.Context, deviceKey string) (*domain.Station, error) {
    var st domain.Station
    if err := r.collection.FindOne(ctx, bson.M{"device_key": deviceKey}).Decode(&st); err != nil { return nil, err }
    return &st, nil
}
...
func (r *stationRepository) FindByID(ctx context.Context, id primitive.ObjectID) (*domain.Station, error) {
    var st domain.Station
    if err := r.collection.FindOne(ctx, bson.M{"_id": id}).Decode(&st); err != nil { return nil, err }
    return &st, nil
}
...
func (r *stationRepository) List(ctx context.Context) ([]*domain.Station, error) {
    ...
    for cur.Next(ctx) {
        var st domain.Station
        if err := cur.Decode(&st); err != nil { return nil, err }
        out = append(out, &st)
    }
