...
func (r *posDeviceRepository) FindByToken(ctx context.Context, token string) (*domain.PosDevice, error) {
    var d domain.PosDevice
    if err := r.collection.FindOne(ctx, bson.M{"device_token": token}).Decode(&d); err != nil { return nil, err }
    return &d, nil
}
...
func (r *posDeviceRepository) FindByID(ctx context.Context, id primitive.ObjectID) (*domain.PosDevice, error) {
    var d domain.PosDevice
    if err := r.collection.FindOne(ctx, bson.M{"_id": id}).Decode(&d); err != nil { return nil, err }
    return &d, nil
}
...
func (r *posDeviceRepository) UpdateConfig(ctx context.Context, id primitive.ObjectID, cardCapable *bool, printerMAC *string, printerUUID *string) error {
    ...
    if cardCapable != nil { set["card_capable"] = *cardCapable }
    if printerMAC != nil { set["printer_mac"] = *printerMAC }
    if printerUUID != nil { set["printer_uuid"] = *printerUUID }
