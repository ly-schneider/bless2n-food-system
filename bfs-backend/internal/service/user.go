...
func (s *userService) RequestEmailChange(ctx context.Context, userID primitive.ObjectID, newEmail, ip, ua string) error {
    ...
    if newEmail == "" { return errors.New("invalid_email") }
    // Load current user
    u, err := s.users.FindByID(ctx, userID)
    if err != nil { return err }
    if u.Email == newEmail { return errors.New("same_email") }
...
func (s *userService) ConfirmEmailChange(ctx context.Context, userID primitive.ObjectID, code string) (*domain.User, error) {
    ...
    if matched.Attempts >= maxEmailCodeAttempts { return nil, errors.New("too_many_attempts") }
    ...
    // Mark token used
    if err := s.emailTokens.MarkUsed(ctx, matched.ID, time.Now().UTC()); err != nil { return nil, err }
    // Update user email and mark verified
    if err := s.users.UpdateEmail(ctx, userID, matched.NewEmail, true); err != nil { return nil, err }
