...
func (s *stationService) RequestVerification(ctx context.Context, name, model, os, deviceKey string) (*domain.Station, error) {
    if name == "" || deviceKey == "" { return nil, errors.New("invalid_payload") }
...
func (s *stationService) VerifyQR(ctx context.Context, code string) (primitive.ObjectID, error) { // nolint: revive
    parts := parseKV(code)
    if parts["v"] != "1" { return primitive.NilObjectID, errors.New("invalid_qr_version") }
    oid, err := primitive.ObjectIDFromHex(parts["orderId"])
    if err != nil { return primitive.NilObjectID, errors.New("invalid_order_id") }
    issuedAtStr := parts["issuedAt"]
    if issuedAtStr == "" { return primitive.NilObjectID, errors.New("invalid_issued_at") }
    ia, err := time.Parse(time.RFC3339, issuedAtStr)
    if err != nil { return primitive.NilObjectID, errors.New("invalid_issued_at") }
    maxAge := time.Duration(s.cfg.Stations.QRMaxAgeSeconds) * time.Second
    if maxAge <= 0 { maxAge = 10 * time.Minute }
    if time.Since(ia) > maxAge { return primitive.NilObjectID, errors.New("qr_expired") }
    // Verify signature
    sig := parts["sig"]
    if sig == "" { return primitive.NilObjectID, errors.New("missing_sig") }
    payload := fmt.Sprintf("v=%s;orderId=%s;issuedAt=%s", parts["v"], parts["orderId"], issuedAtStr)
    if !verifyHMAC(payload, sig, s.cfg.Stations.QRSecret) { return primitive.NilObjectID, errors.New("bad_sig") }
    return oid, nil
}
...
func (s *stationService) AssignedItemsForOrder(ctx context.Context, stationID, orderID primitive.ObjectID) ([]*domain.OrderItem, error) {
    ...
    if err != nil { return nil, err }
    if len(pids) == 0 { return []*domain.OrderItem{}, nil }
...
func (s *stationService) MakePickupQR(ctx context.Context, orderID primitive.ObjectID) (string, error) {
    if orderID.IsZero() { return "", errors.New("invalid_order_id") }
