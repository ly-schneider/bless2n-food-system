...
func (a *authService) RequestOTP(ctx context.Context, email, ip, ua string) error {
	...
	if err := a.emailService.SendLoginEmail(ctx, user.Email, code, ip, ua, otpTTL); err != nil {
		log.Printf("email send failed for %s: %v", user.Email, err)
	}
	return nil
}
...
func (a *authService) VerifyWithCode(ctx context.Context, email, code string, clientID string) (*TokenPairResponse, *domain.User, error) {
	...
	// Accept codes with spaces/hyphens; keep only digits
	if code != "" {
	...
	if !a.limiter.allow(key, 10, 15*time.Minute) {
	...
	log.Printf("OTP verify: email=%s active_tokens=%d", email, len(otps))
	...
	if matched == nil {
		if len(otps) > 0 {
			...
			for i := 1; i < len(otps); i++ {
				if otps[i].CreatedAt.After(latest.CreatedAt) {
					latest = otps[i]
				}
			}
			attempts, _ := a.otps.IncrementAttempts(ctx, latest.ID)
			if attempts >= maxOTPAttempts {
				_ = a.otps.MarkUsed(ctx, latest.ID, time.Now().UTC()) // soft lock that OTP
			}
		}
...
func (a *authService) ListUserActiveSessions(ctx context.Context, userID string) ([]map[string]any, error) {
	...
	for i := range recs {
		...
		if g == nil {
			...
			if r.IssuedAt.Before(g.createdAt) {
				g.createdAt = r.IssuedAt
			}
			if r.LastUsedAt.After(g.lastUsed) {
				g.lastUsed = r.LastUsedAt
			}
		}
	}
