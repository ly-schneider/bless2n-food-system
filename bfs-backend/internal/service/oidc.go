...
func VerifyIDToken(ctx context.Context, tokenString string, jwksURL string, expectedIss []string, expectedAud string, expectedNonce string, cache *jwksCache) (*oidcClaims, error) {
	...
	if ev, ok := mc["email_verified"]; ok {
		oc.EmailVerified = ev
	}
	if name, _ := mc["name"].(string); name != "" {
		oc.Name = name
	}
	if picture, _ := mc["picture"].(string); picture != "" {
		oc.Picture = picture
	}
	if nonce, _ := mc["nonce"].(string); nonce != "" {
		oc.Nonce = nonce
	}
	...
	if oc.Iat > now+60 {
		return nil, errors.New("invalid_iat")
	}
	okIss := false
	for _, iss := range expectedIss {
		if oc.Iss == iss {
			okIss = true
			break
		}
	...
	if !okIss {
		return nil, errors.New("invalid_iss")
	}
	if !audContains(oc.Aud, expectedAud) {
		return nil, errors.New("invalid_aud")
	}
	if expectedNonce != "" && oc.Nonce != expectedNonce {
