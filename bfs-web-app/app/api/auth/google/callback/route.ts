...
export async function GET(req: Request) {
  ...
  const origin = hdrs.get('x-forwarded-host') ? `${secure ? 'https' : 'http'}://${hdrs.get('x-forwarded-host')}` : url.origin
  ...
  const redirectUri = `${hdrs.get('x-forwarded-host') ? (secure ? 'https' : 'http') + '://' + hdrs.get('x-forwarded-host') : new URL(req.url).origin}/api/auth/google/callback`
  ...
  const res = await fetch(`${API_BASE_URL}/v1/auth/google/code`, {
    ...
    body: JSON.stringify({ code, code_verifier: verifier, redirect_uri: redirectUri, nonce: cookieStore.get('g_oauth_nonce')?.value || '' }),
  })
  if (!res.ok) {
    return NextResponse.redirect(mkAbs(`/login?error=oauth_google`))
  }
  ...
  cookieStore.set({ name: 'g_oauth_verifier', value: '', httpOnly: true, secure, sameSite: 'lax', path: '/', maxAge: -1 })
