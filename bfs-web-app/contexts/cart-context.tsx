...
function getStorage(): Storage | null {
  ...
  try {
    // Use sessionStorage on POS route to scope the basket to the session/device
...
function loadCartFromStorage(): Cart | null {
  try {
    const store = getStorage()
    const storedCart = store?.getItem(CART_STORAGE_KEY)
    if (storedCart) {
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const updateItemConfiguration = (oldItemId: string, product: ProductDTO, configuration?: CartItemConfiguration) => {
  ...
  const getTotalProductQuantity = (productId: string): number => {
    return cart.items.filter((item) => item.product.id === productId).reduce((total, item) => total + item.quantity, 0)
  }
